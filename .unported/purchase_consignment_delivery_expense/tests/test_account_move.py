...
class ExpenseOnDeliveryCase(SavepointCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.company = cls.env["res.company"].create({"name": "Some Company"})
        ...
        cls.category = cls.env["product.category"].create({"name": "Consignment"})
        ...
        cls.product_uom = cls.env.ref("product.product_uom_kgm")
        cls.product_po_uom = cls.env.ref("product.product_uom_lb")
