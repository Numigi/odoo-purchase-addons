...
class TestPurchaseOrder(TransactionCase):
    ...
    def test_confirm_po_with_alert_msg(self):
        ...
        self.assertTrue(
            res_confirm["context"]["to_confirm"],
    ...
    def test_approve_po_without_alert_msg(self):
        self.env.user.company_id.write(
            {"po_double_validation": "two_step", "po_double_validation_amount": 300.00}
        )

        self.po_3 = self.PurchaseOrder.sudo(self.user_purchase_user).create(
            self.po_3_vals
        )
        ...
        self.assertTrue(res_confirm, "Purchase: The purchase order has not confirmed")
        self.assertEqual(
            self.po_3.state, "to approve", 'Purchase: PO state should be "to approve".'
        )
    ...
    def test_approve_po_with_alert_msg(self):
        ...
        self.po_4 = self.PurchaseOrder.sudo(self.user_purchase_user).create(
            self.po_4_vals
        )
        ...
        self.assertEqual(
            self.po_4.state, "purchase", 'Purchase: PO state should be "to Purchase".'
        )
