...
class TestInventoryByOwner(SavepointCase):
    ...
    def _add_quant(
        self, product, quantity, location=None, owner=None, lot=None, package=None
    ):
    ...
    def test_inventory_lines_grouped_per_package(self):
        package_1 = self.env["stock.quant.package"].create({"name": "Package 1"})
        package_2 = self.env["stock.quant.package"].create({"name": "Package 2"})
