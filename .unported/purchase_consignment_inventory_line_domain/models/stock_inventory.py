...
class StockInventoryLine(models.Model):
    ...
    def check_selected_product(self):
        ...
        if self.inventory_id.filter == "consignment":
            self._check_product_is_consigned()
    ...
    def _check_product_is_consigned(self):
        if not self.product_id.consignment:
            raise ValidationError(
                _("The product {product} does not have the type consignment.").format(
                    product=self.product_id.display_name
                )
            )
